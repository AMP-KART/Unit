...
import { ref, onValue, push, update, serverTimestamp, get } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-database.js";
    ...
    <div class="space-y-6 w-full max-w-6xl mx-auto pb-10">
        ...
        <div class="flex justify-between items-center hidden md:flex">
            <div>
                ...
                <p class="text-xs text-gray-500 font-bold mt-1">Manage and burn category funds with real-time tracking logs</p>
            </div>
            ...
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between h-40 relative overflow-hidden group hover:border-blue-300 transition-all">
                ...
                <div class="relative z-10">
                    <div class="flex items-center gap-2.5 mb-2">
                        <div class="w-7 h-7 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs"><i class="fa-solid fa-briefcase"></i></div>
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Work Fund (51%)</p>
                    </div>
        ...
        <!-- History & Double Ledgers Section -->
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mt-6">
            <div class="flex items-center justify-between border-b border-gray-100 pb-4 mb-5">
                <div class="flex bg-gray-100 rounded-xl p-1" id="funds-history-tabs">
                    <button class="fund-tab-btn active bg-adminPrimary text-white shadow-sm rounded-lg text-xs font-black px-4 py-2.5 transition-all" data-target="expense-log-wrapper">
                        <i class="fa-solid fa-arrow-up-from-bracket mr-1.5"></i> Expense Log (Burn History)
                    </button>
                    <button class="fund-tab-btn text-gray-500 hover:text-gray-800 rounded-lg text-xs font-black px-4 py-2.5 transition-all" data-target="income-log-wrapper">
                        <i class="fa-solid fa-arrow-down-long mr-1.5"></i> Distribution Log (Inflow)
                    </button>
                </div>
            </div>

            <!-- Tab 1: Expense Log Wrapper -->
                ...
                <div class="overflow-x-auto no-scrollbar rounded-xl border border-gray-100 bg-gray-50/50">
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-white border-b border-gray-100 sticky top-0 shadow-sm">
                            <tr>
                                <th class="py-3 px-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Date & Time</th>
                                <th class="py-3 px-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Category</th>
                                <th class="py-3 px-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Expense Reason/Notes</th>
                                <th class="py-3 px-4 text-xs font-bold text-gray-400 uppercase tracking-wider text-right">Amount Spent</th>
                            </tr>
                        </thead>
                        <tbody id="expense-history-tbody" class="divide-y divide-gray-100">
                            <tr>
                                <td colspan="4" class="py-8 text-center text-sm text-gray-400 font-medium">
            ...
            <!-- Tab 2: Distribution Inflow Log Wrapper -->
...
export const setupAdminFundsEvents = () => {
    ...
    // 2. Fetch Real-time Available Fund Balances
    ...
    // 3. Realtime Expense Log Rendering (Burn Log History)
    onValue(expensesRef, (snapshot) => {
        const tbody = document.getElementById('expense-history-tbody');
        if (!tbody) return;

        if (!snapshot.exists()) {
            tbody.innerHTML = `<tr><td colspan="4" class="py-6 text-center text-xs font-bold text-gray-400 uppercase tracking-wider">No fund expenses burned yet.</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = logs.map(log => {
            ...
            return `
                <tr class="hover:bg-white/80 transition-colors">
                    <td class="py-3 px-4 text-xs font-bold text-gray-700">${new Date(log.timestamp).toLocaleString('en-IN')}</td>
                    <td class="py-3 px-4">
                        <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase border ${catBadgeColors[log.category] || 'bg-gray-100'}">${log.category}</span>
                    </td>
    ...
    // 4. Distribution log rendering (Profit splitter engine allocations history ledger)
    ...
    // 5. Intelligent Multi-Input SweetAlert Burn Action Handler Triggers
    document.querySelectorAll('.btn-burn-fund').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (limit <= 0) {
                Swal.fire({ icon: 'error', title: 'Zero Balance', text: `This ${category} account does not have any active funds to execute burn logs.`, confirmButtonColor: '#0F6B3F' });
                return;
            }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({ title: 'Processing Burn Transaction...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });

                    try {
                        const finalUpdatedBalance = limit - result.value.amount;
                        const updates = {};

                        // Deduct from company ledger node balance seamlessly
                        updates[`company_funds/${category}`] = finalUpdatedBalance;

                        // Create custom burn history log node entries securely
                        ...
                        Swal.fire('Fund Burned!', `₹${result.value.amount} was successfully tracked and deducted from the ${category} fund pool.`, 'success');

                    } catch (err) {
                        Swal.fire('Transaction Failed', 'Firebase operational error. Try executing again.', 'error');
                    }
                }
            });
        });
